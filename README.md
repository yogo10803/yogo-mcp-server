...
一個整合多個 MCP (Model Context Protocol) 伺服器的專案，提供瀏覽器自動化、維基百科搜尋、程式碼文件查詢等工具。此外，還包含一個自動打卡的 Playwright 腳本。
...
- 整合多個實用的 MCP 工具
- 可以一次啟動所有工具
- 包含的工具：瀏覽器自動化（Playwright, Puppeteer）、維基百科深度搜尋（DeepWiki）、程式碼文件查詢（Context7）等
...
## 🌐 如何在 Claude Code 或 Web 版 Claude 中使用？

### 重要說明（白話文版）

**Claude Code（網頁版）和 Claude Web 目前不支援直接連接本地 MCP servers**

為什麼？
- Claude Code 和 Web 版在**雲端**運行
- 你的 MCP servers 在**本地電腦**運行
- 它們之間沒有直接的連接通道
...
### 方式 A：讓 Claude Code 直接執行工具（最簡單）✅

**概念：**
不透過 MCP 協定，直接讓 Claude Code 幫你執行這些工具。

**範例：**

#### 1. 使用 Playwright 瀏覽器自動化

在 Claude Code 對話中說：
```
請幫我寫一個 Playwright 腳本，打開 Google 首頁並截圖
```
...
在 Claude Code 對話中說：
```
請執行 npm run attendance 幫我查打卡記錄
```

Claude Code 會執行腳本並顯示結果。
...
### 方式 B：透過 HTTP 暴露 MCP Server（進階）⚙️

**概念：**
把你的 MCP server 透過 HTTP 對外暴露，讓 Claude Code 可以透過網路連接。

**步驟：**

#### 1. 使用工具暴露本地 server 到網路
...
然後在對話中說：
```
請透過 https://abc123.ngrok.io 連接到我的 MCP server
```
...
### 方式 C：使用 MCP Client 寫一個簡單的介面（最進階）🔧

**概念：**
寫一個簡單的 Node.js 程式，連接你的 MCP servers，並提供 HTTP API。

**步驟：**

#### 1. 建立一個簡單的 MCP Client
...
| 你的需求 | 推薦方式 |
|---------|---------|
| 快速使用，不在意是否透過 MCP 協定 | **方式 A：直接執行** |
| 想要真正的 MCP 連接 | 方式 B：HTTP + ngrok |
| 需要完全客製化 | 方式 C：自己寫 bridge |
...
**Q: 這個專案到底是什麼？**
A: 這是一個工具箱，整合了多個實用的 MCP servers（瀏覽器自動化、維基百科搜尋等），加上一個自動查打卡記錄的小工具。

**Q: 我可以在 Claude Code 或 Web 版中用嗎？**
A: 可以！最簡單的方式是直接請 Claude Code 幫你執行工具，不需要透過 MCP 協定。進階用法可以透過 HTTP tunnel（如 ngrok）來連接。

**Q: 我一定要用全部的功能嗎？**
A: 不用！你可以只用 MCP servers，或只用打卡腳本，或兩個都用。

**Q: MCP 跟一般的 API 有什麼不同？**
A: MCP 是專門為 AI 助手設計的協定。透過 MCP，AI 可以「看到」你提供的工具，並且知道怎麼使用它們。

**Q: 這個專案安全嗎？**
A:
- 打卡腳本需要你的帳號密碼，所以要小心保管 `.env` 檔案（已加入 `.gitignore`）
- MCP servers 跑在你本機，資料不會外傳
- 建議不要把 `.env` 檔案提交到 Git
